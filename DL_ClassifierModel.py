...
class TF-GCN(BaseModel):
    def __init__(self, classNum, embedding, labDescVec,
        ...
        self.BertGCN= models.BertGCN(nb_class=20,m=0.7,gcn_layers=2,n_hidden=200,dropout=0.5).to(device)
        ...
        self.moduleList = nn.ModuleList([self.BertGCN,self.LNandDP,self.icdAttn,self.fcLinear])
    ...
    def calculate_y_logit(self, input):
        ...
        if torch.cuda.device_count() > 1:
            x = nn.parallel.data_parallel(self.BertGCN,x)
